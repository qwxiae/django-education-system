...
from apps.lessons.models import ChoiceStep, ProgrammingStep, Step, TextInputStep

from .models import ChoiceSubmission, CodeSubmission, Submission, TextSubmission
...
def submit_view(request, lesson_id):
    ...
    submission = Submission.objects.create(
        user=request.user, step=step, status=Submission.Status.PENDING
    )
    ...
    if step.type == Step.StepType.TEXT_INPUT:
        ...
        TextSubmission.objects.create(
            submission=submission, answer_text=answer, is_correct=is_correct
        )

        submission.status = (
            Submission.Status.CORRECT if is_correct else Submission.Status.WRONG
        )
        submission.save()
        ...
        choice_step = get_object_or_404(
            ChoiceStep.objects.prefetch_related("options"), pk=step.pk
        )
        ...
        is_correct = set(selected_options.values_list("pk", flat=True)) == set(
            correct_options.values_list("pk", flat=True)
        )

        choice_sub = ChoiceSubmission.objects.create(
            submission=submission, is_correct=is_correct
        )
        ...
        prog_step = get_object_or_404(
            ProgrammingStep.objects.prefetch_related("test_cases"), pk=step.pk
        )
        ...
        transaction.on_commit(lambda: run_code_submission.delay(code_sub.pk))
