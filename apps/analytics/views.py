...
def course_analytics_view(request, slug):
    ...
    avg_progress = (
        Enrollment.objects.filter(course=course).aggregate(avg=Avg("progress"))["avg"]
        or 0
    )

    completed = Enrollment.objects.filter(course=course, progress=100).count()
    ...
    in_progress = Enrollment.objects.filter(
        course=course, progress__gt=0, progress__lt=100
    ).count()

    not_started = Enrollment.objects.filter(course=course, progress=0).count()

    # Most failed steps
    failed_steps = (
        Submission.objects.filter(
            step__lesson__module__course=course, status=Submission.Status.WRONG
        )
        .values("step__title")
    ...
    return render(
        request,
        "analytics/course_analytics.html",
        {
            "course": course,
            # stat cards
...
def teach_dashboard_view(request):
    ...
    return render(
        request,
        "analytics/teach_dashboard.html",
        {
            "courses": courses,
        },
    )
...
def course_create_view(request):
    ...
    return render(
        request, "analytics/course_form.html", {"form": form, "title": "Create Course"}
    )
...
def lesson_create_view(request, slug, module_id):
    ...
    return render(
        request,
        "analytics/lesson_form.html",
        {"form": form, "course": course, "module": module, "title": "Add Lesson"},
    )
...
def lesson_edit_view(request, lesson_id):
    lesson = get_object_or_404(
        Lesson.objects.select_related("module__course"), public_id=lesson_id
    )
...
def step_edit_view(request, step_id):
    ...
    if request.method == "POST":
        ...
        formset = (
            FormSetClass(request.POST, instance=instance) if FormSetClass else None
        )
